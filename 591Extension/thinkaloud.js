...
firebase.database().ref('/thinkaloud').on('value', function(snapshot) {
  if(snapshot.val()!=null)
  {
   ...
   if(data["question"])
   {
    ...
    if(data.value)
    {
    var pushComment = firebase.database().ref("thinkaloud/question/"+id).child('comment').push();
    pushComment.set({
      data: data.value,
   ...
   if(data["explanation"])
   {
    ...
    if(data.value)
    {
    var pushComment = firebase.database().ref("thinkaloud/explanation/"+id).child('comment').push();
    pushComment.set({
      data: data.value,
      user: firebase.auth().currentUser.displayName,
      date: Date()
   ...
   if(data["example"])
   {
    ...
    if(data.value)
    {
    var pushComment = firebase.database().ref("thinkaloud/example/"+id).child('comment').push();
    pushComment.set({
      data: data.value,
      user: firebase.auth().currentUser.displayName,
      date: Date()
