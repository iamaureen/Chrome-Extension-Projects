...
firebase.database().ref('/abtest').on('value', function(snapshot) {
  if(snapshot.val()!=null)
  {
   ...
   if(data["question"])
   {
    ...
    if(data.value)
    {
    var pushComment = firebase.database().ref("abtest/question/"+id).child('comment').push();
    pushComment.set({
      data: data.value,
   ...
   if(data["explanation"])
   {
    ...
    if(data.value)
    {
    var pushComment = firebase.database().ref("abtest/explanation/"+id).child('comment').push();
    pushComment.set({
      data: data.value,
      user: firebase.auth().currentUser.displayName,
      date: Date()
   ...
   if(data["example"])
   {
    ...
    if(data.value)
    {
    var pushComment = firebase.database().ref("abtest/example/"+id).child('comment').push();
    pushComment.set({
      data: data.value,
      user: firebase.auth().currentUser.displayName,
      date: Date()
